      ...
      <div className="absolute inset-4 border-2 border-dashed border-foreground/30 pointer-events-none"></div>
        <header className="flex flex-col gap-6 mb-12">
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div className="font-bold uppercase tracking-wide" style={{ fontSize: '3em' }}>
              <span className="inline-block px-2 py-1 border-4 border-foreground transform -rotate-1" style={{ fontSize: '1em' }}>
                Paper Daily
              </span>
            </div>
          </div>
          <div className="grid gap-5">
            <h1 style={{ fontFamily: "'Gloria Hallelujah', cursive", fontSize: '2.5em' }}>{data.title || 'Sem título'}</h1>
            <p style={{ fontSize: '1.5em' }}>{data.subtitle || ''}</p>
          ...
          {(data.sections || []).map((section, index) => (
            <article key={index} className="p-9 border-4 border-foreground bg-gradient-to-br from-foreground/5 to-transparent relative">
              ...
              {(section.heading || index === 0) && (
                <h2 style={{ fontSize: '1.75em' }}>{section.heading || 'Resumo principal'}</h2>
              )}
